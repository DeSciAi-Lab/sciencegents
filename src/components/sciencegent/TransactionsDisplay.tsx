...
/**
 * Component that displays the token transactions count from Moralis
 */
    ...
    return (
      <TooltipProvider>
        <Tooltip>
          ...
          <TooltipContent>
            <p>Could not fetch transactions count. Click to retry.</p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    );
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <span className={`flex items-center cursor-help ${className}`}>
            {showIcon && <BarChart3 className="mr-1 h-4 w-4 text-gray-400" />}
            {formatNumber(summary.transactionsCount, 0)}
          </span>
        </TooltipTrigger>
        <TooltipContent>
          <p>Token transactions from Moralis</p>
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
